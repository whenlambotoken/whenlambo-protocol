...
contract WhenLambo is BEP20, Ownable {
    ...
    address public _prizePoolAddress;
    ...
    event MaxTxAmountUpdated(uint256 indexed maxTxAmount);
    ...
    constructor(address routerAddress, address payable marketingAddress, address payable prizePoolAddress) public BEP20("WhenLambo", "WHENLAMBO") {
        ...
        setMarketingWallet(marketingAddress);
        setPrizePoolWallet(prizePoolAddress);
    ...
    function setPrizePoolWallet(address payable wallet) public onlyOwner {
        require(wallet != owner(), "Prize pool wallet cannot be the owner");
        _prizePoolAddress = wallet;
        excludeFromFees(_prizePoolAddress, true);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        maxTxAmount = maxSupply.mul(maxTxPercent).div(1000);
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 totalFees = CAKERewardsFee.add(liquidityFee).add(marketingFee).add(prizePoolFee);

            uint256 feeTokens = contractTokenBalance.mul(marketingFee.add(prizePoolFee)).div(totalFees);
        ...
        if (takeFee) {
			uint256 feePercent = CAKERewardsFee.add(liquidityFee).add(marketingFee).add(prizePoolFee);
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 fee = marketingFee.add(prizePoolFee);
        uint256 initialCAKEBalance = IBEP20(CAKE).balanceOf(address(this));

        swapTokensForCake(tokens);
        uint256 toSplit = (IBEP20(CAKE).balanceOf(address(this))).sub(initialCAKEBalance);
        uint256 toMarketing = toSplit.mul(marketingFee).div(fee);
        IBEP20(CAKE).transfer(_marketingAddress, toMarketing);
        IBEP20(CAKE).transfer(_prizePoolAddress, toSplit.sub(toMarketing));
    }
